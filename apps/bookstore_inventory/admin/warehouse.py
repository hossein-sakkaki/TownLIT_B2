...
class WarehouseStaffAssignmentInline(
    admin.StackedInline
):
    model = WarehouseStaffAssignment
    form = WarehouseStaffAssignmentAdminForm

    # Two empty responsibility slots are shown immediately.
    # Django's native "Add another" remains available for additional staff.
    extra = 2
    max_num = 50
    ...
    verbose_name_plural = (
        ...
        "add as many responsible people as needed"
    )
    ...
    fieldsets = (
        ...
        (
            "Operator capabilities",
            {
                ...
                "description": (
                    "Primary managers and managers automatically receive "
                    "all operational capabilities. These switches apply "
    ...
    def get_formset(
        ...
        **kwargs,
        ...
        _disable_user_related_actions(
            formset.form
        )
        return formset

    @admin.display(
        description="Contact from CustomUser"
    )
    def account_contact(self, obj):
        if not obj or not obj.user_id:
            return (
                "Select a user and save to display "
                "the account contact."
            )
...
class WarehouseLocationInline(
    admin.TabularInline
):
    model = WarehouseLocation

    # Useful for setting up several shelves/zones in one edit.
    extra = 2
    max_num = 500
...
class WarehouseAdmin(
    ...
    inlines = (
        WarehouseStaffAssignmentInline,
    ...
    fieldsets = (
        ...
        (
            "Operational responsibility",
            {
                ...
                "description": (
                    "Responsibility and contact details come directly from "
                    "CustomUser accounts through the Staff assignments below. "
                    "You may assign multiple managers or operators to the same "
                    "warehouse. No duplicate personal data is stored here."
                ),
            },
        ),
        (
    ...
    def get_queryset(self, request):
        return (
            ...
            .annotate(
                _total_on_hand=Sum(
                    "balances__on_hand_quantity"
                ),
                _total_reserved=Sum(
                    "balances__reserved_quantity"
                ),
            )
        )
    ...
    def responsible_staff(self, obj):
        assignments = [
            assignment
            for assignment
            in obj.staff_assignments.all()
            if assignment.is_current
        ]

        return ", ".join(
            (
                f"{_user_name(assignment.user)} "
    ...
    @admin.display(
        description="Current managers and operators"
    )
    def responsibility_summary(self, obj):
        if not obj or not obj.pk:
            return (
                "Save the warehouse, then add one or more "
                "responsible CustomUser accounts below."
            )

        assignments = [
            assignment
            for assignment
            in obj.staff_assignments.select_related(
                "user"
            )
            if assignment.is_current
        ]
        ...
        if not assignments:
            return format_html(
                ...
                (
                    ...
                    "Add at least one manager below."
                ),
            )
...
class WarehouseStaffAssignmentAdmin(
    ...
    def get_form(
        ...
        **kwargs,
        ...
        return _disable_user_related_actions(
            form_class
        )
...
def _user_contact(user):
    ...
    return (
        " · ".join(parts)
        or "No email or phone is stored on this account."
    )