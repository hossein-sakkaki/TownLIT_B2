...
class BookAdmin(
    ...
    search_fields = (
        ...
        "subtitle",
        "publisher_name",
        "publisher__official_name",
        "subject_category",
        "contributors__full_name",
    )
    autocomplete_fields = (
        "publisher",
        "rights_holder",
    ...
    fieldsets = (
        ...
        (
            "Publishing and rights",
            {
                ...
                "description": (
                    "Choose organizations from the shared directory. "
                    "Text values are retained as historical snapshots."
                ),
            },
        ),
        (
...
class BookEditionAdmin(
    ...
    fieldsets = (
        ...
        (
            "Publishing and rights",
            {
                ...
                "description": (
                    "Organization fields are reusable directory records; "
                    "name fields are historical snapshots."
                ),
            },
        ),
        (
    ...
    def get_queryset(self, request):
        return super().get_queryset(request).annotate(
            ...
            _unavailable=Sum(
                "balances__unavailable_quantity"
            ),
        )
    ...
    def cover_preview(self, obj):
        if not obj:
            return (
                "Select a book and save "
                "to preview its cover."
            )
    ...
    def stock_summary(self, obj):
        ...
        balances = obj.balances.select_related(
            "warehouse"
        ).order_by(
            "warehouse__name"
        )
        ...
        return " | ".join(
            (
                f"{row.warehouse.name}: "
                f"{row.on_hand_quantity} on hand / "
