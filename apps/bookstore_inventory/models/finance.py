...
class CashLedgerEntry(TimeStampedModel):
    direction = models.CharField(max_length=12, choices=CashEntryDirection.choices, db_index=True)
    entry_type = models.CharField(max_length=32, choices=CashEntryType.choices, db_index=True)
    ...
    ledger_key = models.CharField(
        max_length=180, unique=True, null=True, blank=True, editable=False,
        help_text="Stable idempotency key for automatically synchronized entries.",
    )
