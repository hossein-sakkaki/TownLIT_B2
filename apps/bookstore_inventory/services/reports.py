...
REPORT_DEFINITIONS = OrderedDict((
    (
        "current_inventory",
        {
            ...
            "description": (
                ...
                "quantities by warehouse and edition."
            ),
            "permission": (
                "bookstore_inventory."
                "view_inventorybalance"
            ),
            "dated": False,
        },
    ),
    (
        "stock_activity",
        {
            ...
            "description": (
                "Opening, incoming, outgoing, net, and closing "
                "quantities for a date range."
            ),
            "permission": (
                "bookstore_inventory."
                "view_stockmovement"
    ...
    (
        "movement_detail",
        {
            ...
            "description": (
                "The immutable line-by-line audit trail "
                "for every stock change."
            ),
            "permission": (
                "bookstore_inventory."
                "view_stockmovement"
    ...
    (
        "inbound",
        {
            ...
            "description": (
                "Received shipments, sources, posting state, "
                "acquisition cost, and outstanding amount."
            ),
            "permission": (
                "bookstore_inventory."
                "view_inboundshipment"
            ),
            "dated": True,
        },
    ),
    (
        "orders",
        {
            ...
            "description": (
                "Sales, gifts, donation-based distributions, "
                "fulfilment, and payment status."
            ),
            "permission": (
                "bookstore_inventory."
                "view_bookorder"
    ...
    (
        "transfers",
        {
            ...
            "description": (
                "Stock dispatched and received between "
                "TownLIT warehouses."
            ),
            "permission": (
                "bookstore_inventory."
    ...
    (
        "supplier_payables",
        {
            ...
            "description": (
                "Scheduled, paid, remaining, and overdue "
                "supplier instalments."
            ),
            "permission": (
                "bookstore_inventory."
                "view_inboundpaymentschedule"
            ),
            "dated": True,
        },
    ),
    (
        "cash_ledger",
        {
            "label": "Cash ledger",
            "description": (
                "Cash received and paid, grouped safely "
                "by currency."
            ),
            "permission": (
                "bookstore_inventory."
                "view_cashledgerentry"
            ),
            "dated": True,
        },
    ),
...
def scoped_book_orders_queryset(
    warehouse_ids,
):
    """
    Return only orders whose entire warehouse footprint is accessible.

    This prevents a user assigned to Warehouse A from opening an order that
    also contains hidden Warehouse B lines.
    """
    ...
    return (
        ...
        .annotate(
            _has_inaccessible_warehouse=Exists(
                inaccessible_items
            )
        )
        .filter(
            _has_inaccessible_warehouse=False
        )
        .distinct()
    )


def _parse_requested_date(
    value,
    label,
):
    if not value:
...
def _filters(
    ...
    *,
    ...
    date_from = _parse_requested_date(
        params.get("date_from"),
        "From date",
    )
    date_to = _parse_requested_date(
        params.get("date_to"),
        "To date",
    )
    ...
    if requested_warehouse:
        try:
            ...
            raise ReportValidationError(
                "Warehouse selection is invalid."
            ) from exc

        if warehouse_id not in warehouse_ids:
            raise ReportValidationError(
                "You do not have access to that warehouse."
            )

    movement_type = str(
        params.get("movement_type")
        or ""
    ).strip()
    ...
    valid_movement_types = {
        value
        for value, _label
        in StockMovementType.choices
    }

    if (
        movement_type
        and movement_type
        not in valid_movement_types
    ):
        raise ReportValidationError(
...
def _current_inventory(
    ...
    if filters["query"]:
        ...
        queryset = queryset.filter(
            Q(
                book_edition__book__title__icontains=query
            )
            | Q(
                book_edition__edition_code__icontains=query
            )
            | Q(
                book_edition__edition_name__icontains=query
            )
            | Q(
                book_edition__isbn__icontains=query
            )
            | Q(
                book_edition__barcode__icontains=query
            )
            | Q(
                book_edition__language__icontains=query
            )
        )
    ...
    return _result(
        ...
        totals=(
            ...
            {
                "label": "On hand",
                "value": aggregate["on_hand"],
            },
            {
                "label": "Reserved",
                "value": aggregate["reserved"],
            },
            {
                "label": "Unavailable",
                "value": aggregate["unavailable"],
            },
            {
                "label": "Available",
...
def _stock_activity(
    ...
    queryset = StockMovement.objects.filter(
        warehouse_id__in=warehouse_ids,
        performed_at__date__lte=filters[
            "date_to"
        ],
    )
    ...
    opening_in = Q(
        performed_at__date__lt=filters[
            "date_from"
        ],
    ...
    period = Q(
        performed_at__date__gte=filters[
            "date_from"
        ],
        performed_at__date__lte=filters[
            "date_to"
    ...
    aggregate["opening"] = (
        aggregate["opening_in"]
        - aggregate["opening_out"]
    )
    aggregate["net"] = (
        aggregate["incoming"]
        - aggregate["outgoing"]
    ...
    for item in grouped[:row_limit]:
        ...
        rows.append([
            ...
            item["warehouse__code"],
            item["book_edition__book__title"],
            item["book_edition__edition_code"],
            item["book_edition__language"],
            opening,
            item["incoming"],
            item["outgoing"],
            net,
    ...
    return _result(
        ...
        totals=(
            ...
            {
                "label": "Opening",
                "value": aggregate["opening"],
            },
            {
                "label": "Incoming",
                "value": aggregate["incoming"],
            },
            {
                "label": "Outgoing",
                "value": aggregate["outgoing"],
            },
            {
                "label": "Net",
                "value": aggregate["net"],
...
def _movement_detail(
    ...
    if filters["query"]:
        ...
        queryset = queryset.filter(
            ...
            | Q(
                lot__lot_number__icontains=query
            )
            | Q(
                reference_type__icontains=query
            )
            | Q(
                reference_id__icontains=query
            )
            | Q(
...
def _inbound(
    ...
    if filters["query"]:
        ...
        queryset = queryset.filter(
            Q(
                shipment_number__icontains=query
            )
            | Q(
                supplier_name__icontains=query
            )
            | Q(
                donor_name__icontains=query
            )
            | Q(
                invoice_reference__icontains=query
            )
            | Q(
                notes__icontains=query
            )
    ...
    return _result(
        ...
        currency_totals=_currency_cards(
            currency_groups
        ),
        truncated=(
            total_count > row_limit
...
def _orders(
    ...
    queryset = (
        ...
        .prefetch_related(
            Prefetch(
                "items",
                queryset=scoped_items,
                to_attr="report_items",
            )
        )
    )
    ...
    if filters["query"]:
        ...
        queryset = queryset.filter(
            ...
            | Q(
                recipient_first_name__icontains=query
            )
            | Q(
                recipient_last_name__icontains=query
            )
            | Q(
    ...
    for group in aggregate_queryset.values(
        "currency"
    ).annotate(
        total=Coalesce(
            Sum("total_amount"),
...
def _transfers(
    ...
    if filters["query"]:
        ...
        queryset = queryset.filter(
            ...
            | Q(
                from_warehouse__name__icontains=query
            )
            | Q(
                to_warehouse__name__icontains=query
            )
            | Q(
                notes__icontains=query
            )
    ...
    quantity_totals = queryset.aggregate(
        dispatched=Coalesce(
            Sum("items__quantity"),
            Value(0),
        ),
    ...
    annotated_queryset = queryset.annotate(
        ...
        received_quantity_total=Coalesce(
            Sum(
                "items__received_quantity"
            ),
            Value(0),
...
def _supplier_payables(
    ...
    queryset = _scope_warehouse(
        queryset,
        filters,
        "shipment__warehouse_id",
    )

    if filters["date_from"]:
        queryset = queryset.filter(
            due_date__gte=filters[
                "date_from"
            ]
        )
    ...
    if filters["query"]:
        ...
        queryset = queryset.filter(
            ...
            | Q(
                shipment__supplier_name__icontains=query
            )
            | Q(
    ...
    for schedule in schedules:
        ...
        rows.append([
            ...
            (
                schedule.shipment.supplier_name
                or "—"
            ),
            ...
            (
                ...
                else schedule.get_status_display()
            ),
        ])
...
def scoped_cash_ledger_queryset(
    user,
    warehouse_ids=None,
):
    """
    ...
    Order payments are included only when every warehouse used by that order
    ...
    order_payment_ids = (
        PaymentRecord.objects.filter(
            order_id__in=allowed_orders.values(
                "pk"
            )
        )
        .values_list(
...
def build_report(
    *,
    ...
    available = available_report_definitions(
        user
    )
    ...
    warehouse_ids = report_warehouse_ids(
        user
    )
    ...
    resolved_limit = int(
        ...
        or getattr(
            settings,
            "BOOKSTORE_ADMIN_REPORT_ROW_LIMIT",
            2000,
        )
    )
    resolved_limit = max(
