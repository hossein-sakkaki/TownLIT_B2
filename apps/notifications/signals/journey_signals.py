...
from apps.notifications.services.audience import accepted_friend_users
from apps.notifications.services.presentation import build_journey_message
...
from apps.posts.services.journeys.links import build_journey_entry_link

logger = logging.getLogger(__name__)


def notify_journey_entry_ready(entry: JourneyEntry):
    """
    ...
    Therefore:
    - Entry #1 normally creates and delivers the notification;
    ...
    with transaction.atomic():
        locked_entry = (
            ...
            .select_for_update()
            .select_related("journey", "content_type")
            .filter(pk=entry.pk)
            .first()
        )
        ...
        if locked_entry.notification_dispatched_at is not None:
            logger.info(
        ...
        if not locked_entry.is_live:
            logger.info(
                "[Notif][Journey] Entry is not live; skipped entry=%s",
                locked_entry.pk,
            )
            return

        if locked_entry.visibility == VISIBILITY_PRIVATE:
            logger.info(
                "[Notif][Journey] Private entry; skipped entry=%s",
                locked_entry.pk,
            )
            return
        ...
        if owner_user is None:
            logger.warning(
                "[Notif][Journey] Owner could not be resolved entry=%s",
                locked_entry.pk,
            )
            return
        ...
        if claimed != 1:
            logger.info(
                "[Notif][Journey] Publication claim lost entry=%s",
                locked_entry.pk,
            )
            return

        locked_entry.notification_dispatched_at = claimed_at
        claimed_entry = locked_entry

    recipients = accepted_friend_users(owner_user)
    ...
    message = build_journey_message(owner_user)
    ...
    for recipient in recipients.iterator(chunk_size=200):
        try:
            ...
            create_and_dispatch_notification(
                ...
                # Parent Journey remains the dedupe target.
                target_obj=claimed_entry.journey,

                # Entry ID must not fragment the dedupe key.
                action_obj=None,

                # Navigation still points to the latest exact Entry.
                link=link,
                dedupe=True,
                extra_payload={
                    "publication_event": "journey_entry_available",
                    "journey_id": claimed_entry.journey_id,
                    "journey_entry_id": claimed_entry.pk,
                    "journey_entry_slug": claimed_entry.slug,
                    "journey_sequence": claimed_entry.sequence,
                },
            )
            ...
            logger.exception(
                "[Notif][Journey] Recipient dispatch failed "
                "entry=%s recipient=%s",
                claimed_entry.pk,
    ...
    logger.info(
        ...
        "entry=%s journey=%s processed=%s visibility_skipped=%s failed=%s",
        claimed_entry.pk,
