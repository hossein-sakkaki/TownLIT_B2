...
class StoreSerializer(serializers.ModelSerializer):
    organization = OrganizationReferenceSerializer(
        read_only=True
    )
    ...
    def validate_store_phone_number(
        self,
        value,
    ):
        if (
            ...
            raise serializers.ValidationError(
                ...
                "10 digits and numeric."
            )
        return value

    def validate_license_expiry_date(
        self,
        value,
    ):
        if (
    ...
    def validate(
        ...
        if (
            data.get("is_active")
            and not data.get("license_number")
        ):
            raise serializers.ValidationError(
                "Active store must have "
                "a license number."
            )
        return data