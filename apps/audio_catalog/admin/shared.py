...
def admin_add_url(*, app_label: str, model_name: str, query: str = "") -> str:
...
def linked_object(instance, *, label: str | None = None) -> str:
    """Render an admin object link when available."""

    if instance is None or not getattr(instance, "pk", None):
        return "—"

    text = label or str(instance)
...
def _storage_exists(*, storage, name: str) -> bool:
    try:
        return bool(storage.exists(name))
    except Exception:
        # Some remote storages may still be able to generate a URL.
        return True
...
def _generate_storage_presigned_url(
    *,
    ...
    Direct signing is preferred because a private bucket combined
    with AWS_S3_CUSTOM_DOMAIN may otherwise produce an unsigned URL.
    """
...
def safe_file_url(
    ...
    *,
    ...
    if not _storage_exists(storage=storage, name=name):
        return ""
    ...
    *,
    ...
    return format_html(
        (
            ...
            '<img src="{url}" alt="Artwork preview" loading="lazy" '
...
def render_audio_player(field_file, *, width: int = 320) -> str:
    ...
    resolved_width = max(180, int(width))

    return format_html(
        (
            '<div style="min-width:{width}px;max-width:100%;">'
            '<audio controls preload="metadata" controlslist="nodownload" '
            'style="display:block;width:{width}px;max-width:100%;">'
            '<source src="{url}">'
...
def render_file_link(field_file, *, label: str = "Open file") -> str:
...
def render_json(value: Any) -> str:
    """Render JSON as theme-aware formatted admin content."""
    ...
    return format_html(
        (
            '<pre style="max-width:900px;max-height:420px;overflow:auto;'
            'white-space:pre-wrap;overflow-wrap:anywhere;padding:12px;'
            'border:1px solid var(--border-color);border-radius:8px;'
            'background:var(--darkened-bg);color:var(--body-fg);">{}</pre>'
        ),
        content,
    )
    ...
    *,
    ...
    return format_html(
        (
            ...
            'line-height:1.4;background:{};color:{};">{}</span>'
        ),
        background,
        foreground,
        label,
...
def conversion_status_badge(instance) -> str:
    if getattr(instance, "is_converted", False):
        return status_badge("Ready", background="#18864b")

    return status_badge("Pending", background="#c57a00")


def latest_conversion_job(instance):
    if instance is None or not getattr(instance, "pk", None):
        return None
...
def render_conversion_job(instance) -> str:
    """Render the latest conversion job and progress."""

    job = latest_conversion_job(instance)
    ...
    label = format_html(
        (
            '<span style="display:inline-block;padding:3px 8px;'
            'border-radius:999px;font-size:11px;font-weight:700;'
            'line-height:1.4;background:{};color:#fff;">{} · {}%</span>'
        ),
        color_map.get(job.status, "#666666"),
        job.status,
        job.progress or 0,
    )

    url = admin_change_url(job)
...
class HiddenFromAdminIndexMixin:
    """
    Keep a model registered and directly accessible while hiding it
    from the normal admin application index/sidebar.
    """
