...
class MusicArtwork(MediaAssetsMixin, MediaAutoConvertMixin, MediaConversionMixin, PublicIDTimestampedModel):
    IMAGE = FileUpload("audio_catalog", "artworks", "tracks")
    ...
    track = models.ForeignKey("audio_catalog.MusicTrack", on_delete=models.CASCADE, related_name="artworks")
    role = models.CharField(max_length=20, choices=Role.choices, default=Role.PRIMARY, db_index=True)
    label = models.CharField(max_length=120, blank=True, default="")
    image = models.ImageField(upload_to=IMAGE.dir_upload, max_length=700, validators=[validate_image_file, validate_image_size, validate_no_executable_file])
    ...
    aspect_ratio = models.DecimalField(max_digits=8, decimal_places=5, null=True, blank=True)
    ...
    media_conversion_config = {"image": {"upload": IMAGE, "kind": "image"}}
    ...
    def save(self, *args, **kwargs):
        ...
        if changed and kwargs.get("update_fields") is not None:
            kwargs["update_fields"] = list(dict.fromkeys([*kwargs["update_fields"], "primary_slot"]))
    ...
    def can_deliver_asset(self, *, viewer, field_name, intent):
        ...
        if field_name != "image" or not self.is_available(): return False
        if self.track.is_test_asset: return bool(getattr(viewer, "is_staff", False))
        return self.track.status == self.track.Status.PUBLISHED

    def __str__(self): return f"{self.track.title} · {self.get_role_display()}"

    class Meta:
        ordering = ["sort_order", "-is_primary", "id"]
        indexes = [models.Index(fields=["track", "is_active", "is_primary"]), models.Index(fields=["role", "is_active"])]
        constraints = [models.UniqueConstraint(fields=["track", "primary_slot"], name="audio_one_active_primary_artwork")]
