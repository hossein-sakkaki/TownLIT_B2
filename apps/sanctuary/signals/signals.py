    ...
    try:
        ...
        if created:
            create_and_dispatch_notification(
                ...
                message=f"You have been selected to review a Sanctuary request (#{req.id}).",
                target_obj=req,
                action_obj=review,
    ...
    try:
        ...
        if changed_admin or mode_changed:
            create_and_dispatch_notification(
                ...
                message=f"You have been assigned to review a Sanctuary request (#{req.id}).",
                target_obj=req,
                action_obj=None,
                link=resolve_sanctuary_target_link(req),
                dedupe=True,
            ...
            for u in selected_users:
                ...
                if created:
                    create_and_dispatch_notification(
                        ...
                        actor=req.requester,
                        notif_type="sanctuary_member_review_request",
                        message=f"A Sanctuary request requires your review (#{req.id}).",
                        target_obj=req,
                        action_obj=review,
                        link=resolve_sanctuary_target_link(req),
                        dedupe=True,
...
def _assign_admin_for_outcome_appeal(outcome: SanctuaryOutcome):
    ...
    try:
        ...
        create_and_dispatch_notification(
            ...
            message=f"You have been assigned to review a Sanctuary appeal (Outcome #{outcome.id}).",
            target_obj=outcome,
            action_obj=None,
